...
'#'). For each character they type except '#', you need to return the top 3 historical hot sentences that have prefix the same as 
...
Idea:
Only thing more than a normal Trie is added a map of sentence to count in each of the 
Trie node to facilitate process of getting top 3 results.
        ...
        Queue<Pair> pq = new PriorityQueue<Pair>(new Comparator<Pair>(){
            public int compare (Pair p1, Pair p2){
                if (p1.count == p2.count){
                    return p1.s.compareTo(p2.s);// compareTo for String comparation
                }
                return p2.count - p1.count;
            }
        });
        for (String str : curr.counts.keySet()){
            pq.offer(new Pair(str, curr.counts.get(str)));
        }
