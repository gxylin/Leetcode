...
place parentheses, X1 always goes to the numerator and X2 always goes to the denominator. Hence you just need to maximize Y. 
...
class Solution {
    public String optimalDivision(int[] nums) {
        ...
        if (nums.length == 1){
            return sb.append(String.valueOf(nums[0])).toString();
        }
        if (nums.length == 2){
            return sb.append(String.valueOf(nums[0]) + "/" + String.valueOf(nums[1])).toString();
        }
