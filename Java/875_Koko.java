...
from that pile.  If the pile has less than K bananas, she eats all of them instead, and won't eat any more bananas during this hour.
...
class Solution {
    ...
    private int hrsToFinish(int[] piles, int speed){
        int count = 0;
        for (int pile : piles){
            count += (int)Math.ceil((double) pile / speed);
        }
        return count;
    }
}
