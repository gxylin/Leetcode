...
from start to end. Start and end all have the same format as timestamp. However, granularity means the time level for consideration. 
...
class LogSystem {
    ...
    public List<Integer> retrieve(String s, String e, String gra) {
        ...
        for (String[] strArray : log){
            String str = strArray[1].substring(0, pos);
            if (str.compareTo(s.substring(0, pos)) >= 0 && str.compareTo(e.substring(0, pos)) <= 0){
                res.add(Integer.parseInt(strArray[0]));
            }
        }
        return res;
    }
